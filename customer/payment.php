...
<main class="flex-grow p-4">
  <div class="flex px-20 xl:px-32">
    <div class="w-2/5 px-4 pt-4 mt-4">
      <p class="text-2xl text-neutral-800 font-bold">Order Summary</p>
      <p class="text-gray-400 text-sm">Please check your items before placing the order.</p>
      <div class="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
        ...
        if (isset($_SESSION['email']) && isset($_SESSION['checkoutIds'])) {
          ...
          foreach ($ProductIds as $productId) {
            ...
            $query = "SELECT o.*, p.pd_name, p.pd_img, ct.*
            FROM orders o
            JOIN product p ON o.order_pd_id = p.pd_id
            JOIN customer ct ON o.order_ct_email = ct.ct_email
            WHERE o.order_ct_email = '$email' AND o.order_pd_id = '$productId' AND o.order_code = '$order_code'";
            ...
            if ($result) {
              ...
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                $total_price = $total_price + ($row['order_total_price'] * $qty);
                ?>
                <div class="flex flex-col rounded-lg bg-white sm:flex-row">
                  <img class="m-2 h-24 w-28 rounded-md border object-cover object-center" src="<?php echo $row['pd_img']; ?>"
                    alt="" />
                  <div class="flex w-full flex-col px-4">
                    <span class="font-semibold">
                      <?php echo $row['pd_name']; ?>
                    </span>
                    <span class="float-right text-gray-400 text-sm">Quantity :
                      <?php echo $row['order_pd_qty']; ?>
                    </span>
                    <p class="text-lg font-bold">RM
    ...
    <div class="rounded-lg shadow mt-4 bg-gray-50 px-4 pt-4 mt-4 mb-16 w-3/5">
      <p class="text-2xl text-neutral-800 font-bold">Payment Details</p>
      <p class="text-gray-400 text-sm">Complete your order by providing your payment details.</p>
      <label for="email" class="mt-4 mb-2 block text-neutral-800 font-bold text-sm">Email</label>
      <div
        class="flex items-center rounded-lg border border-gray-200 px-4 py-3 text-sm shadow-sm focus-within:border-blue-500 focus-within:ring-blue-500">
      ...
      <label for="card-holder" class="mt-4 mb-2 block text-neutral-800 font-bold text-sm">Full Name</label>
      <div
        class="flex items-center rounded-md border border-gray-200 px-4 py-3 text-sm shadow-sm focus-within:border-blue-500 focus-within:ring-blue-500">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 mr-2" fill="none" viewBox="0 0 24 24"
          stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div class="flex items-center">
        <input type="text" id="card-no" name="card-no"
          class="w-full flex items-center rounded-lg border border-gray-200 px-4 py-3 text-sm shadow-sm focus-within:border-blue-500 focus-within:ring-blue-500"
          placeholder="xxxx-xxxx-xxxx-xxxx" />
        <input type="text" name="credit-expiry"
          class="w-full rounded-md border border-gray-200 px-2 py-3 text-sm shadow-smfocus:z-0 focus:border-blue-500 focus:ring-blue-500"
          placeholder="MM/YY" />
        <input type="text" name="credit-cvc"
          class="w-1/6 flex-shrink-0 rounded-md border border-gray-200 px-2 py-3 text-sm shadow-sm focus:z-0 focus:border-blue-500 focus:ring-blue-500"
          placeholder="CVC" />
      </div> -->



      <label for="billing-address" class="mt-4 mb-2 block text-neutral-800 font-bold text-sm">Billing Address</label>
      <div
        class="flex items-center rounded-lg border border-gray-200 px-4 py-3 text-sm shadow-sm focus-within:border-blue-500 focus-within:ring-blue-500">
        <svg class="h-4 w-4 text-gray-400 mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
      ...
      <form action="invoice.php" method="post">
        <div class="w-full max-w-3xl mx-auto px-4 lg:px-6 py-8 flex flex-col lg:flex-row">
          <div class="w-full lg:w-1/2 lg:pr-8 lg:border-r-2 lg:border-slate-300">
            <div class="mb-4">
              ...
              <input id="cardNumber" type="text" onclick="hideBackCard()"
                class="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-sm font-normal tracking-more-wider ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:border-blue-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
                maxlength="19" placeholder="XXXX XXXX XXXX XXXX" value="" required />
            </div>
            <div class="flex gap-x-2 mb-4">
              <div class="block">
                <label class="text-neutral-800 font-bold text-sm mb-2 block">Exp. date:</label>
                <input id="expDate" type="text" onclick="hideBackCard()"
                  class="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-sm font-normal tracking-more-wider ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:border-blue-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
                  maxlength="5" placeholder="MM/YY" value="" required />
              </div>
              <div class="block">
                <label class="text-neutral-800 font-bold text-sm mb-2 block">CCV:</label>
                <input id="ccvNumber" type="text" onclick="showBackCard()"
                  class="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-sm font-normal tracking-more-wider ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:border-blue-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
                  maxlength="3" placeholder="123" value="" required />
              </div>
            </div>
            <div class="mb-4">
              ...
              <input id="cardName" type="text" onclick="hideBackCard()"
                class="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-sm font-normal tracking-more-wider ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:border-blue-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
                placeholder="Nurfatin Masyura" value="<?php echo $ct_first_name . ' ' . $ct_last_name; ?>" required />
            </div>
          </div>
          <div class="w-full w-1/2 pl-8 mt-3">
                  ...
                  <div class="w-full absolute top-8">
                    ...
                    <div class="px-8 mt-5">
                      <div class="flex space-between">
                        ...
                        <p id="imageCCVNumber" class="bg-white text-black flex items-center pl-4 pr-2 w-14">
                          342
                        </p>
                      </div>
        ...
        <div class="mt-6 border-t border-b py-2">
          <div class="flex items-center justify-between">
            <p class="text-neutral-800 font-bold text-sm">Subtotal</p>
            <p class="font-semibold text-gray-900">RM
              <?php echo number_format($total_price, 2); ?>
            </p>
          </div>
          <div class="flex items-center justify-between">
            <p class="text-neutral-800 font-bold text-sm">Shipping</p>
            <p class="font-semibold text-gray-900">RM 8.00</p>
          </div>
        </div>

        <div class="mt-6 flex items-center justify-between">
          <p class="text-neutral-800 font-bold text-xl">Total
          <p class="text-gray-400 text-sm mr-auto mt-1 ml-2">(Service Fee Included)</p>
          </p>
          <p class="text-2xl font-semibold text-gray-900">RM
            <?php echo $total_payment = number_format($total_price + 8.00, 2); ?>
          </p>
        </div>

        <input type="hidden" name="pt_date" value="<?php echo date("Y/m/d"); ?>">
        <input type="hidden" name="" value="<?php echo $pro_id; ?>">
        <input type="hidden" name="pt_time" value="<?php echo date("h:i:sa"); ?>">
        <input type="hidden" name="pt_method" value="card">
        <input type="hidden" name="pt_status" value="succeed">
        <input type="hidden" name="pt_total_price" value="<?php echo $total_payment; ?>">
        <input type="hidden" name="pt_ct_email" value="<?php echo $email; ?>">
        <?php
        ...
        ?>
        <button type="submit" class="mt-4 mb-1 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white hover:bg-gray-700">Place
          Order</button>
      </form>
      <script>
